...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    ...
    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(6,182,212,0.4)] border-[#06b6d4]"
      : "border-white/20 hover:border-[#06b6d4]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[#06b6d4]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#06b6d4] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#06b6d4]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
